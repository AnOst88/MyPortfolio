...
<div class="row">
    <div class="col-12">
        <div class="card">
            ...
            <div class="card-body">
                <div class='card-img card-img__max' style="background-image: url({{ asset($post->img) ?? asset('img/1.jpg') }})"></div>
                ...
                <div class="card-btn">
                    ...
                    <form action="{{ route ('post.destroy', ['id' =>$post->post_id]) }}" method="post" onsubmit="if (confirm('Точно удалить пост?')){return true} else {return false}">
                        @csrf
                        @method ('DELETE')
                        <input type="submit" class="btn btn-outline-danger" value="Удалить пост">
